...
export function CandidateManagementFixed() {
  ...
  const loadData = async () => {
    try {
      ...
      console.log("Loaded posts:", postsData.data?.length)
      console.log("Loaded candidates:", candidatesData.data?.length)
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingCandidate) {
        ...
        // Add new candidate
        const { data: newCandidate, error } = await supabase.from("candidates").insert([formData]).select().single()
        ...
        <Card className="border-yellow-200 bg-yellow-50">
          <CardContent className="p-6 text-center">
            <p className="text-yellow-800">
              No positions found. Please initialize the database first using the Database Initializer.
            </p>
          </CardContent>
        </Card>
      )}
